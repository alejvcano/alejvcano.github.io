...
- `vite.config.ts` builds with base path `/website/` when the `GITHUB_PAGES` environment
...
`alejvcano/website` repo you already made. So either:
...
2. On GitHub, rename that repository to `website` (Settings → General → Repository name).
3. The Pages address becomes `alejvcano.github.io/website`, and `vite.config.ts` already
   points at `/website/`, so nothing else needs to change.

---

## 2. Push the code to `alejvcano/website`
...
git remote add origin https://github.com/alejvcano/website.git
...
| CSS and images 404, page looks unstyled | Same as above — assets must resolve under `/website/`. |
| 404 when refreshing `/website/portfolio` | The `404.html` fallback is missing from the `gh-pages` branch. Re-run `bun run deploy`. |
...
| Wrong URL, e.g. `/blank-canvas-start-3586` | The repository name is wrong — rename it to `website` and update `homepage` in `package.json` plus the base path in `vite.config.ts`. |
